    ...
    private static PanelPackets.Snapshot snapshot(ServerPlayer player, Window window, int ack, CacheActions.Result result) {
        ...
        if (record != null) {
            for (int i = 0; i < record.state().cells().size(); i++) {
                ...
                cells.add(new PanelPackets.CellView(cell.filter() == null ? "" : cell.filter().encoded(), cell.amount(), cell.minimum(), cell.maximum(), record.state().pending(i), cell.filter() == null ? 1 : cell.filter().stackSize(), slotResidual));
            }
