...
public final class CacheActions {
    ...
    public static void close(ServerPlayer player) { SESSIONS.remove(player); }
    ...
    private static Result execute(ServerPlayer player, Intent intent, ItemStack creativeCursor) {
        ...
        switch (intent.action()) {
            ...
            case TAKE_CURSOR, TAKE_INVENTORY -> {
                ...
                int available = Math.min(Math.max(0, cell.amount() - CraftingReservations.reservedCache(access.handle().cacheId(), slot, null)), intent.first()); int moved;
