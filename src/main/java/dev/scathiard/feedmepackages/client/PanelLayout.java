...
public record PanelLayout(Rect bounds, List<CellBox> cells, Rect slider, int firstRow, int visibleRows, int totalRows, int footerY, boolean compact) {
    public static final int WIDTH = 46, ROW = 18, HEADER = 24, FOOTER = 6, SLIDER = 22;
    ...
    public static PanelLayout compute(int screenHeight, int left, int top, int count, int firstRow, int expanded, boolean bookOpen) {
        ...
        if (x < 4) return new PanelLayout(new Rect(4, Math.max(4, top), 18, 18), List.of(), null, 0, 0, (count + 1) / 2, 0, true);
        ...
        int rows = Math.max(1, Math.min(Math.min(total, 6), (screenHeight - 8 - HEADER - FOOTER) / ROW));
        int height = HEADER + FOOTER + rows * ROW, y = Math.max(4, Math.min(top, screenHeight - height - 4));
        ...
        return new PanelLayout(new Rect(x, y, width, height), cells, slider, first, rows, total, y + height - FOOTER, false);
    }
}
